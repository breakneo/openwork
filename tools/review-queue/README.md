...
3. Choose **Approve: proposed action**, **Keep** or **Later**. Eligible sessions also offer **Archive** independently of the recommendation; an archive recommendation absorbs it into one Approve control. Later is a browser-local skip: it hides an undecided item from pending/Needs a human, advances focus, persists with the exact snapshot, and never creates a ledger event or owner action. **Later (n)** returns skipped cards; **Return to queue** removes the local skip. One message editor plus **Send to owner** records an independent `message` (offline: Record message). Chips **Why…? / Please change… / Note: / Needs clarification:** only prefill; they never send.
...
Keyboard shortcuts: `j` next, `ArrowUp` previous, `a` approve the proposed action, `x` archive, `k` Keep (`d` remains a legacy Keep alias), `l` Later, `c` focus the message, `Cmd/Ctrl+Enter` send the focused message, `?` prepare clarification, `Space` select. Text inputs/selects retain all typing behavior, and native buttons retain Space activation. Approval shortcuts are blocked while multi-selection is active: use the explicit bulk confirmation.
...
### Recommendation-aware actions and explicit secondary authorization

The primary label is **Approve: <short proposed action>**, derived from the displayed item, not a generic approval label. **Keep** records the legacy `decline` value and displays **Kept**: reviewed, terminal intent with no service mutation, never PR closure. Later remains browser-local. Per the latest keyboard scope, **k now means Keep**, not previous; use ArrowUp for previous and j for next. Text fields retain normal typing/navigation.

**Universal Archive** records `action:"archive"` against an exact session/workspace, independently of review/keep/none recommendation. It requires positive, consistent `Pinned: no` and `Status: idle` evidence, no conflicting Busy/Working/Active user root evidence, no archived/protected state and no hard lock/external ownership. Missing or conflicting safety is not permission. These snapshot checks apply in UI, new decision admission and before claim; fresh applicable live gates still apply to the agent. The complete chat-delivery read gate applies to **both** explicit archive and approve-of-archive, regardless of original recommendation. A canonical archive recommendation uses one absorbed Approve control; if approval prose is absent but exact archive eligibility is established, that absorbed control records explicit archive rather than inventing an approval outcome. Unknown/unavailable proposal approval stays disabled. Worktree-removal references are not session archive permissions.

Secondary destructive controls require native-feed typed human authorization, never text matching in `if_approved`, a proposal, recommendation or summary. Optional `action_authorizations` has at most one entry per supported action:

```json
{"action":"close_pr","target_id":"pr-example","authorized":true,"source":"explicit-human","text":"Explicitly permit closing this exact PR.","provenance":"Synthetic reviewed human instruction","at":"2026-01-01T12:00:00.000Z"}
```

`action` is `close_pr` or `reclaim`; `target_id` must match the exact item and appropriate kind. `authorized` is true/false/null (unknown); only true enables a control. Nonempty reviewed text/provenance and timezone-qualified date are required. Unsupported/malformed declarations reject; missing/false/unknown authorization stays omitted. Summary enrichment and supplement import do not manufacture grants or unlock source locks. Authorization remains a supplied record to recheck, not an instruction inferred by the model from prose.

- **Close PR** appears only with exact typed close permission plus an open, nonprotected PR and validated URL/full captured head. Its confirmation names the PR/head and reviewed permission. It records `close_pr`, not approve, and the conditional plan is `gh pr close '<exact validated PR URL>'` after fresh checks. An approved proposal never implies close authorization. One PR per request; no automatic branch deletion or reopen/rollback. A `closed` result is a terminal report, not independently verified effect.
- **Reclaim** appears only for an authorized actual worktree with an exact absolute path, not a short/unknown identity. It records **approve only**, and displays the entire POSIX-quoted `git worktree remove -- '<exact path>'` command; embedded apostrophes are escaped, no force/delete-recursive flags are added. Recheck owning repository, worktree registration, ownership, cleanliness and activity before any execution. One target per request. Explicit `locked:true`, external ownership/prefixes and protection remain hard bounds; a grant does not override them. Historical imported worktrees remain explicitly locked unless the owner deliberately supplies a different reviewed native snapshot.

`archive` shares archive-result/unarchive compensation handling with approve-of-archive. It never changes the original recommendation. Close PR/reclaim are not magically reversible; previously recorded effects and unknown outcomes remain in the audit. All actions use the same authenticated snapshot-bound request namespace, idempotency and claim/withdrawal lock.

### Locked / another owner's items

`group: "external-mission"`, external mission prefixes and `locked:true` are non-bypassable. `lock_reason` can explain why; neither `locked:false`, a recommendation nor a typed grant overrides external ownership. Worktree categories/removal references remain read-only unless an actual worktree has the exact typed Reclaim permission described above and no hard lock. The supplement importer still explicitly locks worktrees. `none` is not an archive veto for an otherwise positively safe session, nor a veto of an explicit permitted PR Close/Reclaim; without such eligibility it remains read-only. Original recommendation text is never rewritten into authorization.
...
The offline generator emits conservative agent-initiated conditional plans, never executes them. Historical/imported archive approvals remain readable, but their exported plans apply the shared `canArchive` guard (including Busy, Working and Active user root observations) before the stricter offline scope checks. These offline plans are not the LIVE per-item human-approval authority described under Incident attention below:
...
- **Keep (legacy decline)/defer:** no service mutation. Keeping a PR never closes it; keeping a worktree never removes it.
- **Explicit archive:** emits the exact conditional `session.archive` intent independently of recommendation, with positive admission evidence, complete chat read acknowledgement and fresh applicable human-archive gates. This is distinct from conservative historical offline approve-of-archive recommendations above.
- **Close PR / Reclaim:** only the typed, exact-target operations described above produce their fixed conditional commands. They are not inferred from proposal approval or prose.
...
The executor should record export identity, effective decision ID/batch/time, live precondition results, action attempted, outcome, and resulting URL/state. Stop on an uncertain outcome rather than retrying through another route. The offline generator does not execute services; the optional LIVE claim/result protocol is documented below and still delegates external actions to the authorized agent.
...
### Owner objections — point-by-point response

This table maps the requested objections to the implemented contract and synthetic proof. “Changed design” refers to the cumulative branch implementation, including earlier hardening commits; #6 additionally changes the action model, typed authorization and dialect. “Clarified” does not add permission. This is **not deployment approval** and contains no live incident identifiers or counts.

| Owner point | Design change or clarification | Resolution | Synthetic verification |
|---|---|---|---|
| Executor 1 — claim/withdrawal race and recovery | Changed design and expanded proof | Canonical data-directory ledger lock covers withdrawal/dependency recomputation, publication repair, result transition and fsynced claim. No child is exposed until its entire frozen envelope is published. | `review-queue-controls.test.ts`: withdrawal-first, claim-first and concurrent CLI/HTTP races; every prefix of queued/completed undo/change envelopes; legacy publication hole; unequal duplicate/torn rejection. |
| Executor 2 — rechecking is a claim, not inactivity/completion | Clarified and enforced | A persisted claim remains potentially running/unresolved. No elapsed-time or parentless-root inference. Replies do not replace the execution status or make running work interruptible. | Controls spec: rejected running undo even after a reply, at-most-once claim; incidents spec: accepted/claimed/waiting/terminal states and unknown descendants. |
| Executor 3 — queued receipts are not processed actions | Changed UX, clarified counting | Input receipts establish acceptance only. Current outcomes are per request/item; historical/repeated blocks do not multiply incidents. Waiting and replies are not done. | `review-queue-incidents.test.ts` and `review-queue-messages.e2e.test.ts`: latest-state counts, history retention and filtered log; no replay control. |
| Executor 4 — freeze/drain, epoch and stop latch | Changed protocol, clarified operations | Sentinel/metadata gate rejects incompatible readers before new claims. Freeze ingress and drain owners at a tool boundary; reconcile claimed/waiting, back up integrity metadata, restart same SHA/data directory, verify epoch, preserve stop latch. No rollout occurs here. | Controls spec: old reader shape rejected, unchanged epoch on restart, stop remains latched. |
| Compensation — reviewed effect and exact success | Changed design | Compensation references the original effect receipt and exact reviewed follow-up text. Every target needs structured success before replacement. Later independent archive blocks reversal; out-of-band generation must be checked live. Unknown/mixed targets require reconciliation. | Controls/actions specs: structured single/bulk success, partial targets, original-effect mismatch/later archive, message cannot unsend, direct archive uses guarded unarchive. |
| Server 1 — authentication, namespace and reused validation | Retained boundary; new actions validated | Same token/Host/Origin/Fetch Metadata/CSP and exact body/snapshot validation. Shared request namespace includes control children/read events. Typed Close/Reclaim permissions are exact-target; no natural-language authorization inference or bulk merge/close/reclaim. | Live/actions specs: auth/route denials, reuse conflicts, stale snapshots, typed target matching, no inferred closure and exact-head single-PR replacement. |
| Server 2 — absolute data directory, not code directory | Clarified and canonicalized | Source checkout locates code only. `privateDirectory` resolves the actual absolute data directory; its lock is shared across aliases/processes. The owner must pass the agreed absolute LIVE `--dir`, never substitute this feature checkout or its default reports directory. | Live/controls specs: isolated owned directories, cross-process lock exclusion, explicit CLI directory and privacy constraints. |
| Server 3 — frozen ordered recovery | Changed design | One append stores the ordered control/child plan and stable IDs. Shared repair validates existing records first and appends only missing equal events, on retry and before claim. No regeneration or ledger rewriting. | Controls spec: all publication prefixes and idempotent recovery, corrupted/unequal receipts block. |
| Server 4 — bounded port and EADDRINUSE | Changed interface | Optional integer `--port` 0–65535; default0/random, IPv4 loopback only. Occupied fixed port fails explicitly; no fallback. | Controls/live specs: bounds, fixed-port restart, occupied port error and default random loopback. |
| Server 5 — no magic rollback | Clarified with guarded controls | Withdrawal wins only before claim. Running cannot be interrupted; completed archive needs new verified compensation; follow-up cannot unsend or stop work. Merge/PR closure/reclaim have no automatic rollback. No replay button. | Controls/actions/browser specs: running/irreversible/unknown denials, compensation ordering, explicit Close and approve-only Reclaim records. |

### Protocol epoch and maintenance window (no automatic rollout)

Server startup under `.server.lock` and the canonical directory's `.ledger.lock` appends exactly one sentinel `{kind:"protocol",protocol:4,epoch:UUID}` in `decisions.jsonl` and saves identical private `protocol.json`. This sentinel intentionally has **no event**: the old executor's all-input publication validation fails before any claim, and old server writers cannot interpret it. New readers validate sentinel/metadata agreement before filtering it from work. Missing sentinel with existing metadata, unequal epochs, unknown versions and multiple sentinels fail closed. A crash after sentinel append can finish metadata creation only during coordinated server startup. Authenticated `GET /protocol` exposes the version/epoch; the new UI requires protocol4. The sentinel does **not** interrupt an old tool that was already claimed/running. Dialect4 adds archive/close actions and typed destructive authorization; dialect3 readers must not claim those inputs. An existing dialect3 epoch is deliberately rejected, not silently rewritten or cleared. Unversioned legacy lines remain readable under a newly initialized dialect4 epoch. Any already-versioned migration requires a separately reviewed owner procedure; none is performed by this code or task.
...
### Incident attention and deterministic verification

**Blocked (n)** and **Waiting (n)** are page-level links to the filtered action log. Counts use the latest effective result per request/item, not the number of historical attempts. Duplicate/repeated blocked receipts count once; a later archive result removes that incident while the log retains **Historical blocked** evidence. Per-target outcomes can leave one batch target blocked and another completed. Card status, blocker text and Needs a human visibility use that card's matching outcome, never the aggregate batch result. A reply is discussion, not an execution-status transition: it cannot erase rechecking, blocked, waiting or archived. Accepted/queued is distinct from persisted claim/rechecking, which remains potentially running/unresolved without a terminal receipt. Waiting is never done. There is no replay button.

The pure `effectiveActionStatuses(events)` drives those counts and latest-verification lines. It preserves message/disposition separation and per-target results. `tools/review-queue/gates.mjs` provides `evaluateArchiveGates(observation)` and `waitingReceipt(...)`; neither reads services nor dispatches external actions. The authorized executor supplies fresh observations and must honor `allowed:false` before any tool call. Tests prove the evaluator policy, not real external execution.

The archive evaluator requires an observation timestamp, exact session identity verification, an explicit authority (`live-human` with matching `approved_item_id` and `approved_action:"archive"`, or `agent-initiated` plus a confirmed standing grant), and explicit safety values. In fixed order it checks authorization, identity, pinned, exact active-user-root match, external mission/prefix, busy, working, descendant state and unresolved safety. Unknown fails closed. **`parentId:null` alone does not establish that a root is the operator's active root.** The active-root comparison must be observed directly; missing comparison is unknown.

**LIVE per-item human approval is an explicit instruction for the frozen action, not an agent-initiated recommendation.** It can authorize a cross-workspace archive without a scope-only block. It does not override pins, the operator's active root, external mission ownership, busy/working sessions or descendants, identity mismatch, unresolved safety, the chat read gate, or merge-specific exact-head gates. Approval is not proof of effect.

Only agent-initiated archive applies standing-grant scope and actual owned-task-worktree cleanliness (plus purpose/learnings/pending/remaining-work review). Reporting/watchdog work with **no owned task worktree** is N/A, not clean and not blocked by the coordinator's dirty worktree. Never substitute the coordinator worktree for task ownership. Unknown/other ownership for code work remains blocked. Human approval does not require these agent-only scope/worktree heuristics.

A blocked evaluator result includes deterministic **first failing gate**, `observed_at`, one-line `who` and `unblock`, in both fields and its formatted `text`; record that text with `result ID blocked TEXT`. Waiting receipts require `owner`, `outstanding`, `observed_at`, `next_check_at` after the observation, and explicitly say waiting is not done. Old free-text receipts remain readable but do not gain missing verification fields or safety certainty. No timer or idleness inference changes a claim's state.
...
2. Treat `archive` as the exact explicit human archive instruction, not approval of whatever recommendation happened to be on the item. It uses the same applicable live gates as approve-of-archive. `close_pr` is only the explicitly permitted exact PR-close operation; worktree `approve` is Reclaim only with the validated grant and displayed exact command. Keep remains `decline` and never mutates a service. Unknown action meanings are blocked, never guessed. A returned `stop` is prioritized: append `result ID stopped TEXT`, stop processing, and require a fresh operator instruction before resuming. The protocol does not kill in-flight tools or erase other pending inputs. Check for newly queued stop events between consequential tool calls; do not fetch-and-ignore extra work since `next` claims it.
3. Before acting, re-read live identity, ownership, pins, the exact active user-root identity, busy/working/descendant state and all action-specific safety gates. External missions, excluded owners and locked items remain untouched. Apply the explicit-human versus agent-initiated distinction above: do not scope-only block an authorized human archive or substitute coordinator dirt for an absent owned task worktree. Agent-initiated work additionally needs purpose complete, learnings captured, no pending decision, no remaining task work and applicable owned-task-worktree checks. Unknown applicable safety means `blocked` with first gate, observation time and who/unblock, not guessed permission.
...
5. Write a result after each inspected outcome. For owner questions, confirm the exact authorized recipient before using `session.send`; record `waiting` with owner, outstanding question, observed time and next-check time. **Monitor waiting owners manually** through session affordances, then record `reply`/a final status against the original request ID. `next` does not poll owners, relaunch them or automatically replay waiting work.
