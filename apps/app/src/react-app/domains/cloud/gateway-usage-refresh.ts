import { gatewayUsageStatusSchema } from "@openwork/types/den/gateway-usage-limits";
...
export function createGatewayUsageSettlementRefresh(input: {
  ...
  const step = async (attempt: number) => {
    ...
    if (!current()) { running = false; return; }
    const nextAttempt = restartRequested ? 0 : attempt;
    restartRequested = false;
    if (nextAttempt >= gatewayUsageSettlementDelays.length || (nextAttempt > 0 && !input.hasUnresolved())) {
      running = false;
      return;
    }
  ...
  if (active?.scope !== scope) {
    ...
    const runner = createGatewayUsageSettlementRefresh({
      ...
      hasUnresolved: () => client.getQueryCache().findAll({ queryKey: gatewayUsageQueryPrefix, type: "active" }).some((query) => {
        ...
        return query.state.status === "error" || !result.success || !result.data.coverage.complete
          || result.data.coverage.unpricedRequests > 0 || (result.data.coverage.quarantinedRequests ?? 0) > 0;
      }),
