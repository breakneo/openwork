...
export const server = async () => ({ tool: {
  ...
  browser_act: operationTool("act", "Dispatch one action against a fresh observation under this thread's browser-control grant, without another per-action prompt. Scroll uses viewport CSS coordinates and a distance from -1200 to 1200: positive down, negative up. Permission does not authorize unrelated or consequential tasks. It returns a dispatch receipt, not task success. Observe and verify; never repeat uncertain actions automatically.", { tabId, observationId: z.string(), action }),
