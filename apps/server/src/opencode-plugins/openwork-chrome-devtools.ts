...
export const server = async () => ({ tool: {
  ...
  browser_act: operationTool("act", "Dispatch one action against a fresh observation. Every click, fill and key requires separate user confirmation; scrolling uses the thread's grant without another prompt. Scroll uses viewport CSS coordinates and a distance from -1200 to 1200: positive down, negative up. It returns a dispatch receipt, not task success. Observe and verify; never repeat uncertain actions automatically.", { tabId, observationId: z.string(), action }),
