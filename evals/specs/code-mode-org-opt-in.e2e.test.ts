...
const test = spec.world(async (seed) => {
  // The opt-in is inert without the deployment switch; this journey exercises
  // the enabled path. The default-off path is covered by the API unit tests.
  const den = await seed.den({ web: true, env: { OPENWORK_EVAL_MYSQL8: "1", DEN_CODE_MODE_OPT_IN_ENABLED: "true" }, org: { name: "Code Mode journey", members: {
