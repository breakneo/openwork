...
test('live review posts explicit decisions, polls correlated threads and keeps private data behind the token', async ({ evidence }) => {
  ...
  try {
    ...
    expect(followup?.action).toBe('ask_info');
    await page.getByTestId('thread-input').fill('Keep this draft while polling');
    await page.getByTestId('thread-input').focus();
    ...
    expect(await page.getByTestId('thread-input').inputValue()).toBe('Keep this draft while polling');
    expect(await page.getByTestId('thread-input').evaluate((node) => node === document.activeElement)).toBe(true);
    ...
    for (const format of ['json', 'markdown']) {
      ...
      if (format === 'json') {
        ...
        expect(audit.decisions).toHaveLength(5);
    ...
    expect(await page.getByTestId('archive-batch').count()).toBe(0);
    await page.getByTestId('thread-input').fill('stop');
    await page.getByTestId('thread-send').click();
...
test('unconfirmed local audit and both draft types survive reload without becoming server decisions', async ({ evidence }) => {
  ...
  try {
    ...
    await page.getByTestId('thread-input').fill('Unsent owner draft');
    ...
    expect(await page.getByTestId('thread-input').inputValue()).toBe('Unsent owner draft');
    expect(posts).toBe(0);
...
test('human filtering follows terminal dispositions and surfaces blocked archive and keep results', async ({ evidence }) => {
  ...
  try {
    ...
    expect(await page.getByTestId('thread-input').inputValue()).toBe('Draft kept as the row disappears');
    expect(await page.getByTestId('archive-batch').count()).toBe(0);
    await page.locator('#status').selectOption('');
    await page.locator('[data-id="ses_keep"] .row-open').click();
    await page.getByTestId('thread-input').fill('Please inspect the kept reference');
    await page.getByTestId('thread-send').click();
