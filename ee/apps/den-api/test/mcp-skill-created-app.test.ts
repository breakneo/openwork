...
test("lists skill CRUD tools without confirmation bindings while retaining Workflow Artifact resources", async () => {
...
test("create_skill returns schema-valid structured content and text without presentation metadata", async () => {
...
test("update_skill preserves write annotations without a resource binding", async () => {
  await withClient(async (client) => {
    const tools = await client.listTools()
