...
test("save and retirement refresh the same session's resources and tools", async () => {
  ...
  await withClient(async (client) => {
    ...
    expect(tools.tools.find((tool) => tool.name === `render_artifact_${viewId}`)?._meta)
...
test.each(draftDataModes)("%s draft save validates legacy preview metadata without activation and retains explicit standard preview", async (dataMode) => {
  ...
  await withClient(async (client) => {
    ...
    loadData: async () => {
      loads += 1
      return { ok: true, payload, markdown: "# Preview" }
    },
    activate: async () => {
      activations += 1
      return view
...
test("saving a compiled draft rejects unavailable preview data without legacy ready metadata", async () => {
  ...
  await withClient(async (client) => {
    ...
    loadData: async () => {
      loads += 1
      return { ok: false, error: "capability_unavailable", message: "Connect your account", connectionCard }
    },
  })
})
