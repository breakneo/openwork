...
export function registerAgentGeneratedArtifactViews(input: {
  ...
  if (input.readSource) {
    ...
    input.server.registerTool("read_artifact_view", {
      ...
      description: "Organization owners and admins only. Read the newest draft source of an app you manage before improving it. Use the app id from its render or preview tool name. Editing must keep the existing artifactViewId and configObjectId.",
  ...
  input.server.registerTool(
    "save_artifact_view",
    {
      ...
      description: [
        "Organization owners and admins only; being the app creator or a Workflow manager does not override this requirement. Create or improve an in-app dashboard or artifact view of Workflow results. Call this Cloud MCP tool directly, not through search_capabilities or execute_capability. Compile React source into a self-contained immutable MCP App revision bound to one Workflow output schema.",
  ...
  input.server.registerTool(
    "activate_artifact_view_revision",
    {
      ...
      description: "Organization owners and admins only. Point an Artifact's render tool at an exact compatible immutable revision. Selecting an older revision performs a rollback without changing its bytes.",
  ...
  input.server.registerTool(
    "retire_artifact_view",
    {
      ...
      description: "Organization owners and admins only. Remove the active render capability without deleting or changing any immutable view revision.",
