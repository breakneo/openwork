...
export function registerAgentGeneratedArtifactViews(input: {
  ...
  if (input.readSource) {
    ...
    input.server.registerTool("read_artifact_view", {
      ...
      description: "Restricted to organization owners and admins when administrator-only app management is enabled for this deployment. Read the newest draft source of an app you manage before improving it. Use the app id from its render or preview tool name. Editing must keep the existing artifactViewId and configObjectId.",
  ...
  input.server.registerTool(
    "activate_artifact_view_revision",
    {
      ...
      description: "Restricted to organization owners and admins when administrator-only app management is enabled for this deployment. Point an Artifact's render tool at an exact compatible immutable revision. Selecting an older revision performs a rollback without changing its bytes.",
  ...
  input.server.registerTool(
    "retire_artifact_view",
    {
      ...
      description: "Restricted to organization owners and admins when administrator-only app management is enabled for this deployment. Remove the active render capability without deleting or changing any immutable view revision.",
