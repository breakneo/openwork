...
export function registerOrgWorkflowRoutes<T extends { Variables: OrgRouteVariables }>(app: Hono<T>) {
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Grants the teammate identified by email viewer access to the app's underlying Workflow and places the app on their personal dashboard; result data is never copied. An existing editor or manager grant for that teammate is kept, so repeated shares never downgrade access. Requires manager access to the Workflow and, when administrator-only app management is enabled for this deployment, an organization owner or admin and an app with an active saved revision; fails with teammate_not_found when no active member of the organization has that email.",
      responses: {
        200: jsonResponse("App shared to the teammate's dashboard.", z.object({ ok: z.literal(true) })),
        403: jsonResponse("Only app managers can share; organization owners and admins only when administrator-only app management is enabled.", forbiddenSchema),
  ...
  app.post(
    ...
    describeRoute({
      ...
      description: "Restricted to organization owners and admins when administrator-only app management is enabled for this deployment. Adds (added: true) or removes (added: false) the app on the calling member's personal dashboard. Adding requires an app with an active saved revision that the caller can read; removal also works after access to the app has been revoked. Both directions are idempotent.",
      responses: {
