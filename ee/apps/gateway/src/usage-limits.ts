...
export type CheckGatewayUsage = (
  input: GatewayUsageScope & GatewayUsagePreflight & { requestId: string },
...
export const checkGatewayUsage: CheckGatewayUsage = async (input) => {
  try {
    ...
    const admission = await createGatewayUsageLimits(db).admit(scope, input.requestId, accountable)
