...
test("member Auto requests join the organization's Gateway request log while guests stay per device", async () => {
  const rows: Array<Record<string, unknown>> = []
  const f = fixture({ usageLog: { insert: async (row) => { rows.push({ ...row }) }, update: async (row) => { rows.push({ ...row }); return true } } })
  const memberResponse = await f.app.fetch(signed(MEMBER_FREE_CHAT_PATH, `Bearer ${memberKey}`, prompt))
  assert.equal(memberResponse.status, 200)
  await memberResponse.json()
  await new Promise((resolve) => setTimeout(resolve, 50))
  assert.equal(rows.length, 2, "one pending insert and one settled update")
  const [pending, settled] = rows
  assert.equal(pending.route, "openwork_openrouter")
  assert.equal(pending.organization_id, member.organizationId)
  assert.equal(pending.org_membership_id, member.memberId)
  assert.equal(pending.inference_key_id, null)
  ...
  assert.equal(settled.input_tokens, responseValue().usage.prompt_tokens)
  assert.equal(settled.output_tokens, responseValue().usage.completion_tokens)
  assert.ok(typeof settled.cost_micro_usd === "number" && settled.cost_micro_usd > 0)
  assert.equal(settled.upstream_request_id, "generation-1", "the provider generation id is the durable usage identity")
...
test("member Auto fails closed when the Gateway request log cannot be written, without consuming allowance", async () => {
  const f = fixture({ usageLog: { insert: async () => { throw new Error("accounting offline") } } })
  const response = await f.app.fetch(signed(MEMBER_FREE_CHAT_PATH, `Bearer ${memberKey}`, prompt))
  assert.equal(response.status, 503)
  assert.equal((await response.json()).error.code, "request_log_unavailable")
  assert.equal(f.calls.fetch, 0, "nothing was sent upstream")
