...
function checkExpression(value: string) {
  ...
  let text = value.replace(/_utf8mb4\\'(member:|team:|organization)\\'/g, "'$1'")
    .replace(/`(?:gateway_provider_access|gateway_usage_bucket|gateway_usage_consumption_event|gateway_usage_limit_assignment|gateway_usage_limit_entry)`\./g, "").replace(/`|\s/g, "")
    .split(/('[^']*')/).map((part, index) => index % 2 ? part : part.toLowerCase()).join("")
    .replace(/_utf8mb4(?=')/g, "")
    .replace(/\(((?:org_membership_id|member_id|team_id|cost_micro_usd)is(?:not)?null)\)/g, "$1")
