...
--> statement-breakpoint
CREATE TABLE `inference_free_keys` (
	`id` varchar(64) NOT NULL,
	`membership_joined_at` timestamp(3) NOT NULL,
	...
	CONSTRAINT `inference_free_keys_id` PRIMARY KEY(`id`),
	CONSTRAINT `inference_free_key_member` UNIQUE(`org_membership_id`),
	CONSTRAINT `inference_free_key_hash` UNIQUE(`key_hash`)
);
--> statement-breakpoint
...
CREATE TABLE `inference_free_usage_buckets` (
	`id` varchar(64) NOT NULL,
	`scope` enum('member','installation','ip','global') NOT NULL,
...
CREATE INDEX `inference_free_reservation_expiry` ON `inference_free_reservations` (`status`,`expires_at`);