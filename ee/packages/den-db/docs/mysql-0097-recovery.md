...
3. Use a native writable primary: supported parser bounds are MySQL **8.0.16
   ...
   Do not bypass the guard: validate that exact platform on an isolated restored
   copy and obtain a reviewed update first. These bounds are compatibility
   guards, **not evidence every version or managed service was tested**.
...
6. Obtain the reviewed source checkout and install its locked repository
   ...
   **This source script is not present in the released 0.18.48 image.** No build
   entry or Helm hook is installed by this change. Do not point an existing image
   at a nonexistent recovery binary or assume production-pruned dependencies
   can run it. Use the reviewed checkout in a secured operator environment.
...
`sslaccept=strict`, or `sslmode`/`ssl-mode` set to `require`, `verify-ca` or
`verify-full`; all are upgraded to full identity verification. Unknown,
duplicate, conflicting alias or insecure options are rejected, including
`ssl=false`, `sslaccept=accept_invalid_certs` and `rejectUnauthorized=false`.
...
The **28 recovery unit tests** and **4 harness unit tests** passed without
...
receipt ordering and ambiguous-response handling with synthetic executors.
