...
This demo uses **Den Web to create and share a managed dashboard**, then two isolated desktops to view it. It does not save a chat result or copy a Workflow snapshot. The three real MCP Apps are Acme Home, World Clocks, and Personal Calendar.
...
Run from the **release-based demo checkout**, not the current development product tree. The prepared local checkout is `/Users/guillaume/Dev/openwork-eng105-release` (branch `demo/eng-105-release`). Follow its documented toolchain and world prerequisites. Install both workspaces with `pnpm install --frozen-lockfile` and `pnpm --dir evals install --frozen-lockfile`. Use only the world-owned Den URL, desktop CDP endpoints, and separate browser profiles. Never attach to a personal running desktop.

**Required product build: released OpenWork v0.18.46**, source SHA `a0d6bd1de8debf4f09d22b8538e124b2ff45b339`. This script targets the shipped UI: dashboard tiles have a visible header **Refresh** button, not the newer compact App-options menu. The world reports `releaseTag`, `releaseSha`, and `lane: local-release-source` and refuses product-source differences from this tag. The harness cannot inject released images into the required seeded topology, so this is the approved exact-release-source fallback, **not packaged-binary verification**. A later development checkout is not release proof.
...
| Personal Calendar | Pending verified deployment receipt; use world outputs | Planned `show_calendar {}`; deployment verification pending | OAuth / per-member; scope `calendar:read` |
...
A subsequent **release-source** smoke on overlay `0ffdedc85` booted the exact v0.18.46 product tree in 1m49s, with Den API/Web ports **8790/3005** and signed-in `alex@acme.test` / `jordan@acme.test` desktops on CDP ports **55559/55810**. The portable script created all three connections with HTTP **201**, then reapplied them with HTTP **200**, unchanged IDs and empty changed-field lists; setup exit codes were `[0,0]`. Calendar was `oauth/per_member`, organization-wide, and **not yet connected for either member**. That world is also stopped. This proves local boot/configuration readiness, not OAuth isolation, full E2E completion, or packaged-binary compatibility.
...
| 3 | Open the organization's connection inventory and inspect Acme Home, World Clocks, and Personal Calendar. | All three expose an MCP App. Calendar is **Individual accounts / per-member** with OAuth. Its `ui://` resource is served by the Vercel app, not a generated Workflow view. |
| 4 | As Alex, open **My Library → MCPs → Personal Calendar**, which opens **Your Connections**, and click **Connect**. | The demo OAuth redirect returns without asking for a password or choosing a person. Alex's connection reports connected. A failed OAuth return is a failed step, not permission to use shared credentials. |
...
| 8 | Add Personal Calendar with default launch arguments `{}` and enable **Auto-run**. | The calendar is a real MCP App tile bound to the per-member connection. Its launch arguments contain no identity, bearer token, or shared calendar payload. |
...
| 13 | As Jordan, open **My Library → MCPs → Personal Calendar → Your Connections → Connect**. | OAuth auto-approves Jordan's separate connection and returns connected. No API key, password, or identity picker is required. |
...
| 16 | On Alex's calendar, click the visible tile-header **Refresh** button (accessible name **Refresh Personal Calendar**). | A new tool invocation with the same stored `{}` arguments returns a generation greater than `A.generation`. Alex's identity and meeting set remain Alex's; a mere iframe repaint is insufficient. |
