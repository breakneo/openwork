...
|---|---|---|---|
| A — managed dashboard + automatic member OAuth | Required dashboard-demo user journey | Passed | Named sharing; distinct member calendars; identity-stable refresh with disclosed per-instance generation semantics. Independent D observed steps 1–19 Passed; step 20 exposed the same known limitation. |
...
| C — actual CDP film | Same-run presentation | Passed within approved 9/10 presentation scope | Final caption-v3 60-second PARTIAL film exported and verified. Required dashboard-demo result leads; no all-tests-pass claim. |
...
Fork **(b)** was selected after exact CLI error `Error: The specified scope does not exist` and original-project response projection `{"status":403,"error":"forbidden"}`. The approved fallback is **https://world-clocks-demo.vercel.app/mcp**, deployment `<deployment ID>` under the deployment operator. Initialize/tools/resource reads returned 200, with seven tools/four bindings. Only tonight's owned configuration changed. The original Clock deployment and real-org connector remain untouched. Rollback tag `pre-resource-binding` points to `fc86788f0aad5f9ee083f6fdca4a67c43929b74c`; legacy subtree code is aligned at `451d2d2cd39330b62286cbec7dc38947b2697508`.

## Morning actions — in priority order

1. **Demo operator: Connections → World Clocks (`<connection ID>`) → URL `https://world-clocks-demo.vercel.app/mcp` → Refresh tools (7)**, then verify Edit/save. Alternatively the deployment administrator grants the ops token original `<deployment team ID>` access so the canonical fix can be deployed there. The demo agent did not edit that connector.
2. Decide visibility and organization transfer for three repositories: Acme Home public, Personal Calendar private, World Clock Dashboard local. The demo operator decides; transfer to the target organization needs the organization administrator. No public Calendar publication or org transfer occurred.
...
- Production shared preferences survived a forced second Vercel deployment, with fresh `{}` returning `storeKind: blob`. Existing one-shot released-host spec then retained newly added Paris: exit 0, 1 passed/0 failed/0 skipped, all ten claim groups Passed on `2db17ef65`. Historical evidence pointer withheld; retained in the private audit.
- New C 48s / D 50s COMPLETE films use only privately retained same-run captures, original finalized runner hash, and owner/media bindings. Historical players and receipts are withheld pending media privacy review; original pointers are retained only in the private audit. Media bytes have not been scrubbed. Prior PARTIALs unchanged.
...
- World ports 8790/3005 were stopped and verified free, then explicitly handed to the per-user Home proof owner for its authorized per-user Home proof. That lane owns boot/leave-alive; this Blob lane will not restart it. Audit handled copying COMPLETE films into the protected main checkout.
