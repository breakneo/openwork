...
Once a connected App is open, it owns its action controls: clicking **Send**, for example, does not open an additional OpenWork approval modal. This also permits background tool calls from that App; it is not a click-only permission. Calls still require authenticated collaborator access, organization permissions, the originating server, and a live launch lease. Read-only previews cannot call tools.
...
- Writable Apps advertise `serverTools` and `openLinks`. Tool calls retain the originating endpoint, workspace, session, engine (when supplied), and live launch lease through dispatch, without an additional host confirmation. Link requests require an active writable view, use the host's HTTP(S)-only opener, and report failures.
...
- Mark read-only tools with `annotations: { readOnlyHint: true }`. Launch tools without it, or with `destructiveHint: true`, are treated as modifying data and run only on request unless an admin enables auto-run. This launch rule is separate from calls made by an already-open App.
