...
The cost view adds known costs recorded for each request. Gateway uses a provider-reported cost when available; otherwise, it estimates cost from reported token usage and available model pricing. Stored estimates are not recalculated when catalog prices change.

Known cost can appear even if token usage is unavailable. Missing cost estimates are excluded rather than treated as free. These figures are approximate and may differ from your provider's invoice, including because of contract pricing or usage that Gateway could not observe.
