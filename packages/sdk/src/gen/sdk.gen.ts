...
export class DenClient extends HeyApiClient {
   ...
   *
   * Restricted to organization owners and admins when administrator-only app management is enabled for this deployment. Adds (added: true) or removes (added: false) the app on the calling member's personal dashboard. Adding requires an app with an active saved revision that the caller can read; removal also works after access to the app has been revoked. Both directions are idempotent.
   */
   ...
   *
   * Retires the Artifact view: its status becomes retired, it loses its active revision and useInWorkflow flag, and it is removed from every member's dashboard. Immutable revisions are kept, so activating one later restores the view. Requires manager access to the Workflow and, when administrator-only app management is enabled for this deployment, an organization owner or admin.
   */
